...
// Immediately-Invoked Function Expression to handle GAPI client loading
// This now includes a retry mechanism to ensure 'gapi' is defined.
(function initializeGapiClientOnAppLoad() {
    console.log('app.js initiating GAPI client core loading...');

    const checkGapiReady = () => {
        // Check if the global 'gapi' object and its 'load' method are available
        if (typeof gapi !== 'undefined' && gapi.load) {
            console.log('gapi object found. Loading the core client module...');
            gapiCoreLoadedPromise = new Promise(resolve => {
                gapi.load('client', resolve); // Proceed with gapi client loading
            });

            gapiCoreLoadedPromise.then(async () => {
                console.log('gapi.client core module loaded successfully.');
            ...
            console.log('gapi object not yet found. Retrying in 100ms...');
            // If gapi is not ready, wait a short period and try again
            setTimeout(checkGapiReady, 100);
        }
    };

    checkGapiReady(); // Start the checking process
})(); // The parentheses here immediately execute the function
